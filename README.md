...
- You can also run the `Release` workflow manually, provide an existing tag, and rebuild assets from that exact tagged revision.
...
- 이미 만든 태그에도 **Actions > Release > Run workflow**로 다시 실행해, 그 태그 시점 기준으로 실행 파일 자산을 다시 붙일 수 있습니다.
